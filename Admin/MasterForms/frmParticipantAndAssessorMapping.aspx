        ...
        function fnDSEList() {
            ...
            PageMethods.fngetdata(CycleId, CycleDate,assessmenttypeid, function (result) {
                ...
                if (result.split("|")[0] == "2") {
                    ...
                    $("input.clsDate").datepicker({
                        minDate:new Date(CycleDate),
                        changeMonth: true,
                        changeYear: true,
                        dateFormat: "dd-M-yy",
                        showOn: "button",
                ...
                if ($(this).find("#ddlassessor").val() > 0) {
                    ...
                    if (OldParticipantCycleMappingId != $(this).attr("ParticipantCycleMappingId")) {
                        ...
                        if ($(this).find("#ddltimeslot").val() != "0") {
                            timeselected = 1;
                            startTime = assessmenttypeid == 1 ? $(this).attr("cycledate") : $(this).find("input.clsDate").val().replace(/-/g, " ") + " " + $(this).find("#ddltimeslot").val();
                            startTime = startTime.replace(/-/g, "/");
                            startTime = new Date(startTime);
                        } 
                    }
                    CycleDate = assessmenttypeid == 1 ? CycleDate : $(this).find("input.clsDate").val().replace(/-/g, " ")
                    if (timeselected == 1) {
                        ...
                        var eBreakTime = 0;//parseInt(exerciseID) == 3 ? 15 : 0;
                        var stime = parseInt(PrepTime) + parseInt(AssessorTime) + parseInt(eBreakTime);
                        var endTime = new Date(startTime);
                        endTime = new Date(startTime.getTime() + stime * 60000);


                        var Hours = endTime.getHours();
                        var Minutes = endTime.getMinutes();
                        Minutes = Minutes == "" ? "00" : Minutes;
                        var entime = Hours.toString().trim() + ":" + Minutes.toString().trim();


                        
                        ArrDataSaving.push({ ParticipantCycleMappingId: $(this).attr("ParticipantCycleMappingId"), AssessorCycleMappingId: $(this).find("#ddlassessor option:selected").attr("AssessorCycleMappingId"), ParticipantId: $(this).attr("ParticipantId"), AssessorId: $(this).find("#ddlassessor").val(), IsLeadAssessor: IsLeadAssessor, ExerciseId: $(this).attr("ExerciseId"), ExerciseName: $(this).attr("ExerciseName"), AssesseMeetingLink: "", AssessorMeetingLink: "", MeetingId: "0", MeetingStartTime: CycleDate + " " + sstime, MeetingEndTime: CycleDate + " " + entime, PreTime: $(this).attr("PreTime"), AssessorTime: $(this).attr("AssessorTime"), AssessorMail: $(this).find("#ddlassessor option:selected").attr("AssessorEmailId"), AssessorName: $(this).find("#ddlassessor option:selected").attr("AssessorName"), AssesseeMail: $(this).attr("participantemailid"), AssesseeName: $(this).attr("participantname"), BEIUserName: $(this).find("#ddlassessor option:selected").attr("BEIUserName"), BEIPwd: $(this).find("#ddlassessor option:selected").attr("BEIPassword"), flgCreated: 0, MeetingStatus: '', gdid: 0, PRStartTime: "00:00",Role:Role });
                        cntOneTonOne++;
                        OldParticipantCycleMappingId = $(this).attr("ParticipantCycleMappingId");
                        
                        var sBreaktime = parseInt(exerciseID) == 2 ? 15 : 0;
                        startTime = new Date(endTime);
                        startTime = new Date(startTime.getTime() + sBreaktime * 60000);
                    } else {
                        IsCnt++;

                        ArrDataSaving.push({ ParticipantCycleMappingId: $(this).attr("ParticipantCycleMappingId"), AssessorCycleMappingId: 0, ParticipantId: $(this).attr("ParticipantId"), AssessorId: 0, IsLeadAssessor: 0, ExerciseId: $(this).attr("ExerciseId"), ExerciseName: $(this).attr("ExerciseName"), AssesseMeetingLink: "", AssessorMeetingLink: "", MeetingId: "0", MeetingStartTime: CycleDate + " 00:00", MeetingEndTime: CycleDate + " 00:00", PreTime: "0", AssessorTime: '0', AssessorMail: '', AssessorName: $(this).find("#ddlassessor").val() == "0" ? "" : $(this).find("#ddlassessor option:selected").text(), AssesseeMail: $(this).attr("participantemailid"), AssesseeName: $(this).attr("participantname"), BEIUserName: '', BEIPwd: '', flgCreated: 0, MeetingStatus: 'Mapping Pending', gdid: 0, PRStartTime: "00:00",Role:Role });
                    }
            ...
            if (result.split("|")[0] == "0") {
                ...
                if (tbl.length > 0) {
                    ...
                    for (var i in tbl) {
                        ...
                        var Role = tbl[i]["Role"];
                        var rowspancnt =  Role == "Level3" ? 5 : (Role == "Level4" ? 4 : 1);
                        var gdid = tbl[i]["gdid"];
                        var strColor = "";
                        if (flgCreated == 0) {
                            flgvalid = false
                        ...
                        if (Oldparticipantcyclemappingid != participantcyclemappingid) {
                                ...
                                strHTML += "<td  style='" + style + ";text-align:center;padding:3px'  rowspan='" + rowspancnt + "'>" + cnt + "</td>";
                                strHTML += "<td  style='" + style + ";text-align:left;padding:3px'  rowspan='" + rowspancnt + "'>" + ParticipantName + "</td>";
