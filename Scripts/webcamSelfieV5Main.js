...
function fnSaveContinue() {
    ...
    var BandId = 1;//$("#ConatntMatter_hdnBandId").val();
    var PageName = $("#ConatntMatter_hdnFolderName").val();
    var inLoginid = document.getElementById("ConatntMatter_hdnLoginID").value;
    inLoginid = (inLoginid == undefined ? 0 : inLoginid);
    ...
    if (inLoginid > 0) {
        ...
        $.ajax({
            ...
            success: function (result) {
                $("#pageload").hide();
                if (result.split("|")[0] == "1") {
                    window.location.href = "../Exercise/MyTask.aspx?intLoginID=" + inLoginid
                    //window.location.href = "../Questionnaire/frmLifeRefQuestionnaire.aspx?RspID=" + rspId + " & intLoginID=" + inLoginid
