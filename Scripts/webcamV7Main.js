...
function stopWebcam() {
    if (webcamStream != undefined) {
        ...
        audio.src = '';
        if (eventStartSnapFunction != "") {
            window.clearInterval(eventStartSnapFunction);
            clearTimeout(eventStartSnapFunction);
        }
...
var eventStartSnapFunction="";

function fnStatProctoring() {
    ...
    eventStartSnapFunction = setInterval(function () {
        snapshot();
    }, 300000);
