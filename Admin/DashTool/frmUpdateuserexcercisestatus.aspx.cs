...
public partial class frmUpdateuserexcercisestatus : System.Web.UI.Page
{
    ...
    public static string Gettbl()//string AssessorId
    {
        ...
        Scmd.CommandText = "spGetUserListForPearsonExercise";
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            ...
            sb.Append("<table cellpadding='0' cellspacing='0' valign='middle' id='clsStoretbl'  style='width:100%' >");
            sb.Append("<thead>");
                ...
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    for (int j = 0; j < ds.Tables[0].Columns.Count; j++)
                    {
                        if (!SkipColumn.Contains(ds.Tables[0].Columns[j].ColumnName.ToString().Trim()))
                        {
                            if (j > 2)
                            {
                                if (ds.Tables[0].Rows[i][j].ToString().Split('^')[1] == "1")
                                {
                                    sb.Append("<td><a target='_Blank' id=" + ds.Tables[0].Rows[i][j].ToString().Split('^')[2] + " onclick='clicklist(this);'  >" + ds.Tables[0].Rows[i][j].ToString().Split('^')[0] + "</a></td>");
                                }
                                else
                                {
                                    sb.Append("<td><div id=" + ds.Tables[0].Rows[i][j].ToString().Split('^')[2] + " >" + ds.Tables[0].Rows[i][j].ToString().Split('^')[0] + "</div></td>");
                                }

                            }
                            else
                            {
                                sb.Append("<td>" + ds.Tables[0].Rows[i][j].ToString() + "</td>");
                            }
                        }
                    }
                    sb.Append("</tr>");
    ...
    [System.Web.Services.WebMethod()]
    public static string UpdatepearsonExerciseStatus(string RSPExerciseId)
    {
        ...
        try
        {
            ...
            SqlCommand Scmd = new SqlCommand();
            Scmd.Connection = Scon;
            Scmd.CommandText = "spUpdatepearsonExerciseStatus";
            Scmd.CommandType = CommandType.StoredProcedure;
            //Scmd.Parameters.AddWithValue("@tblAssmentRsltDetails", DttblRating);
