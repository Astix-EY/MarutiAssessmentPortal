...
public partial class M3_Rating_RatingStatus : System.Web.UI.Page
{
    ...
    private static string createStoretbl(DataSet ds, int headerlvl, bool IsHeader)
    {
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
                    ...
                    if (!SkipColumn.Contains(dt.Columns[j].ColumnName.ToString().Split('|')[0].Trim()))
                    {
                        ...
                        if (ColSpliter[k] != "")
                        {
                            if (string.Join("", ColSpliter) == ColSpliter[k])
                            {
                                ...
                                {
                                    sb.Append("<th rowspan='" + ColSpliter.Length + "' colspan='2' style='text-align:center'>Action</th>");
                                }
                                else
                                {
            ...
            foreach (DataRow Row in ds.Tables[0].Rows)
            {
                ...
                int Row_Span1 = 0;
                DataRow drows1 = null;
                Row_Span1 = dt.Select("[Participant Code]='" + Row["Participant Code"].ToString() + "'").Count();
                drows1 = dt.Select("[Participant Code]='" + Row["Participant Code"].ToString() + "'")[0];
                string flfgOverallStatus = Row["flgOverallStatus"].ToString();
                if (flfgOverallStatus != "2")
                {
                    if (dt.Select("[Participant Code]='" + Row["Participant Code"].ToString() + "' and Action<>''").Length > 0)
                    {
                        if (Row_Span1 > 1)
                        {
                            if (Row == drows1)
                            {
                                sb.Append("<td style='text-align:center;vertical-align:middle' rowspan='" + Row_Span1 + "' ><input type='button' value='Final Submit' class='btn btn-primary' onclick='fnFinalSubmit(" + Row["RspExerciseID"].ToString() + ")' /></td>");
                            }
                        }
                        else
                        {
    ...
    {
        using (SqlConnection Scon = new SqlConnection(ConfigurationManager.ConnectionStrings["strConn"].ConnectionString))
        {
